...
class MainActivity:Activity(){
 ...
 override fun onCreate(b:Bundle?){super.onCreate(b);t=JenTts(this);v=JenVoice(this)
 ...
 setContentView(l)}
 fun listen(){status.text="LISTENING";v.listen({q->chat.text="You: $q\n\nJen: thinking...";local(q)},{say(it)})}
 ...
 fun ai(q:String){val e=Prefs.endpoint(this);if(e.isBlank()){say("GPT backend URL save karo.");return};status.text="THINKING";Executors.newSingleThreadExecutor().execute{try{
  ...
  val j=JSONObject(c.inputStream.bufferedReader().use{it.readText()});runOnUiThread{chat.text="You: $q\n\nJen: "+j.optString("reply");t.say(j.optString("reply"));status.text="READY";j.optJSONObject("action")?.let{execute(it)}}}catch(_:Exception){runOnUiThread{say("AI server se connection nahi ho pa raha.")}}}}
 ...
 fun say(s:String){status.text="READY";chat.text="Jen: $s";t.say(s)}
